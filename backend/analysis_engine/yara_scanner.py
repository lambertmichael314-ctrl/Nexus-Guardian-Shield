...
class YaraScanner:
    """YARA rule scanner wrapper for CTI Platform malware analysis.

    Compiles all .yar files in ``yara_rules/`` at startup and provides
    ``scan_file()`` for synchronous scanning.  Designed to be called from
    ``tasks.py`` as an additional analysis layer alongside the Python
    detector modules.
    """
    ...
    def _compile_rules(self) -> None:
        ...
        try:
            ...
            # Extract rule names for diagnostics
            ...
            logger.exception("Unexpected error compiling YARA rules")
            self.rules = None

    def scan_file(self, file_path: str) -> Dict[str, Any]:
        """Scan a single file with all compiled YARA rules.

        Returns a dict compatible with the detector result format:
            {
                "detected": bool,
                "confidence": float,
                "details": {"matches": [...]},
...
def analyze(target_path: str) -> Dict[str, Any]:
    """Entrypoint for ``tasks.py`` subprocess dispatch."""
    scanner = YaraScanner()
